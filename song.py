...
class Song:
	...
	def loadAnnotSegments_fixNegativeStart(self):
		...
		if self.segment_indices[0] < 0:
			...
			self.beats = [i*beat_length_s for i in range(int(self.beats[0] / beat_length_s))] + self.beats;# logger.info(self.songBeginPadding); logger.info(self.onset_curve)
	...
	def open(self):
		...
		self.loadAnnotDownbeat()
		self.loadAnnotGain()
		# print("open song", self.title)


	def openAudio(self):
		filename_1 = os.path.join(self.dir_, self.title + self.extension);
		sound = AudioSegment.from_file(filename_1, format="mp3")
		sound = np.array(sound.get_array_of_samples())
		sound = sound.reshape((-1, 2))
		self.laudio = sound[:, 0]
		self.raudio = sound[:, 1]
		self.laudio = np.float32(self.laudio) / 30000
		self.raudio = np.float32(self.raudio) / 30000
		self.audio = np.column_stack((self.laudio, self.raudio))

		# self.dir_ = self.dir_ + "/" + self.title # cause the spleets are one level down in the directory
		#
		# filename_1 = os.path.join(self.dir_, "bass.mp3");
		# filename_1 = os.path.join(self.dir_, "bass.mp3");
		# sound = AudioSegment.from_file(filename_1, format="mp3")
		# sound = np.array(sound.get_array_of_samples())
		# sound = sound.reshape((-1, 2))
		...
		# self.laudio = np.float32(self.laudio) / 30000
		# self.raudio = np.float32(self.raudio) / 30000
		# self.bass = np.column_stack((self.laudio, self.raudio))
		#
		# filename_1 = os.path.join(self.dir_, "drums.mp3");
		# sound = AudioSegment.from_file(filename_1, format="mp3")
		# sound = np.array(sound.get_array_of_samples())
		# sound = sound.reshape((-1, 2))
		# self.laudio = sound[:, 0]
		...
		# self.laudio = np.float32(self.laudio) / 30000
		# self.raudio = np.float32(self.raudio) / 30000
		# self.drums = np.column_stack((self.laudio, self.raudio))
		#
		# filename_1 = os.path.join(self.dir_, "other.mp3");
		# sound = AudioSegment.from_file(filename_1, format="mp3")
		# sound = np.array(sound.get_array_of_samples())
		# sound = sound.reshape((-1, 2))
		# self.laudio = sound[:, 0]
		...
		# self.laudio = np.float32(self.laudio) / 30000
		# self.raudio = np.float32(self.raudio) / 30000
		# self.other = np.column_stack((self.laudio, self.raudio))
		#
		#
		# filename_1 = os.path.join(self.dir_, "vocals.mp3");
		# sound = AudioSegment.from_file(filename_1, format="mp3")
		# sound = np.array(sound.get_array_of_samples())
		# sound = sound.reshape((-1, 2))
		# self.laudio = sound[:, 0]
		...
		# self.laudio = np.float32(self.laudio) / 30000
		# self.raudio = np.float32(self.raudio) / 30000
		# self.vocals = np.column_stack((self.laudio, self.raudio))

	# filename_1 = os.path.join(self.dir_, "piano.mp3");
		# sound = AudioSegment.from_file(filename_1, format="mp3")
		# right_sound = sound.split_to_mono()[1]
		# left_sound = sound.split_to_mono()[0]
		# self.laudio = normalizeAudioGain(np.array(list(array.array('h', left_sound._data)), dtype='single') / 30000,self.replaygain)
		# self.raudio = normalizeAudioGain(np.array(list(array.array('h', right_sound._data)), dtype='single') / 30000,self.replaygain)
		# self.piano = np.column_stack((self.laudio, self.raudio))
