...
def get_key_transposed(key, scale, semitones):
	if key == 'Bb': #chain of ifs and elifs hack is because annotation can result in a Bb but the 'notes' list defined above only understands C#
		idx = 10
	elif key == 'Eb':
		idx = 3
	elif key == 'Fb':
...
class SongCollection:
	...
	def init_key_title_map(self): #counts how many songs in which key to print at the beginiing
		self.key_title = {}
	...
	def get_annotated(self):
		# in case I want the whole list of BPMs:
		# for s in self.songs:
		# 	s.open()
		# 	print s.title,':', s.tempo, '\n'
