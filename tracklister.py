...
TRANSITION_PROBAS = {
	TYPE_CHILL : [1.0, 0.0, 0.0],	# chill -> chill, rolling, ddrop
	TYPE_ROLLING : [1.0, 0.0, 0.0], # rolling> chill, rolling, ddrop
	TYPE_DOUBLE_DROP : [1.0, 0.0, 0.0] # ddrop -> chill, rolling, ddrop
	}
...
def getAllSlaveCues(song, fade_type, min_playable_length = 48): #TODO min playable length!!
	'''
	...
	if fade_type == TYPE_DOUBLE_DROP:
		...
		cues = LH #[indices[0] + LENGTH_CHILL_IN] # i did this
		fade_in_lengths = [LENGTH_CHILL_IN]
	else:
		raise Exception('Unknown fade type {}'.format(fade_type))
		
	return cues, fade_in_lengths
...
def getSlaveQueue(song, fade_type, min_playable_length = 32):
	...
	# print song.title
	cue, fade_in_len = cues
	cue = cue[0]
	fade_in_len = fade_in_len[0]
	return cue, fade_in_len
...
class TrackLister:
	...
	def getFirstSong(self):
		...
		firstSong = self.songsUnplayed[0]#np.random.choice(self.songsUnplayed, size=1)[0]
	...
	def getUnplayed(self):
		unplayed = self.song_collection.get_annotated()
	...
	def filterSongOptionsByThemeDistance(self, song_options, master_song):
		...
		return self.songsUnplayed  #song_options[song_options_closest_to_centroid[:NUM_SONGS_ONSETS]]
	...
	def getBestNextSongAndCrossfade(self, master_song, master_cue, master_fade_in_len, fade_out_len, fade_type):
		...
		for s in song_options:
			...
			best_master_cue = master_cue_corr
				...
				# 	best_slave_cue_clash = queue_slave_cur# 	next_song.title[:20],
				# 	fade_type,
				# 	queue_slave_cur,
				# 	score,
